...
{
    const auto worldPos = camera->getViewProjectionInv() * glm::vec3(ndcCoords, 1.0f);
    return worldPos;
}

glm::vec2 RenderContext::worldToNdc(const glm::vec3 worldCoords) const
{
    const auto ndcPos = camera->getViewProjection() * worldCoords;
    return ndcPos;
}
...
glm::vec2 RenderContext::pixelToWorld(const glm::ivec2& pixelCoords) const
{
    ...
    const auto worldCoords = camera->getViewProjectionInv() * glm::vec3(ndcCoords, 1.0f);
    return glm::vec2(worldCoords.x, worldCoords.y);
}

glm::ivec2 RenderContext::worldToPixel(const glm::vec2& worldCoords) const
{
    const auto ndcCoords = camera->getViewProjection() * glm::vec3(worldCoords, 1.0f);
